...
def db_documents(passport_copy, residence_card_front, residence_card_back, passport_photo):
    args = [passport_copy, residence_card_front, residence_card_back, passport_photo]
    blobs = [blobbify(each) for each in args]
    c.execute(f"insert into document(passport_copy, residence_card_front, residence_card_back, passport_photo) values(?, ?, ?, ?);", blobs)
    db.commit()
