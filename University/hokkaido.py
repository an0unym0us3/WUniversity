...
hokkaido = ctk.CTk()
hokkaido.title('Hokkaido University')
ws = hokkaido.winfo_screenwidth() 
hs = hokkaido.winfo_screenheight() 
w = 1000 
h = 750 
x = (ws/2) - (w/2) 
y = (hs/2) - (h/2) 
hokkaido.geometry('%dx%d+%d+%d' % (w, h, x, y))
ctk.set_appearance_mode('light')

def open_window():

    main_frame = Frame(hokkaido)
